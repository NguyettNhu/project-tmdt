...
export default function ProductCard({ id, name, price, originalPrice, image, rating = 0, reviewCount = 0 }: ProductCardProps) {
                ...
                <div className="flex items-center gap-1.5 mb-4">
                    <div className="flex gap-0.5">
                        {[...Array(5)].map((_, i) => (
                            <svg
                                key={i}
                                className={`w-4 h-4 ${i < Math.round(rating) ? 'text-amber-400 drop-shadow-sm' : 'text-gray-200'}`}
                                fill="currentColor"
                                viewBox="0 0 20 20"
                            >
