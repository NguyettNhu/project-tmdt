...
export default function BagsPage() {
  ...
  // Filter bags products
  const bagsProducts = useMemo(() => {
    const filtered = products.filter(product => product.category === 'Bags');
  ...
  return (
    <div className="w-full bg-gray-50 min-h-screen">
      {/* Hero Section */}
      <section className="relative bg-linear-to-br from-[#F59E0B] via-[#FBBF24] to-[#FDE68A] py-16 lg:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center">
            ...
            <p className="text-xl text-white/90 max-w-2xl mx-auto">
              Bộ sưu tập túi sách thời trang cao cấp
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        ...
        <div className="flex items-center justify-between mb-8 bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <p className="text-gray-600">
            <span className="font-semibold text-gray-900">{bagsProducts.length}</span> sản phẩm
          </p>
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as 'featured' | 'price-low' | 'price-high' | 'name')}
            className="px-4 py-2 border-2 border-gray-200 rounded-xl focus:border-[#F59E0B] focus:outline-none transition-colors font-medium"
          >
        ...
        {bagsProducts.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-6">Sản phẩm túi sách sẽ sớm được cập nhật</p>
            <Link
              href="/shop"
