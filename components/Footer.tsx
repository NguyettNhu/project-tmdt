// components/Footer.tsx
"use client";

import Link from "next/link";
import { useState } from "react";
import { Check } from "lucide-react";

export default function Footer() {
  const [email, setEmail] = useState("");
  const [isSubscribed, setIsSubscribed] = useState(false);

  const handleSubscribe = (e: React.FormEvent) => {
    e.preventDefault();
    if (email) {
      setIsSubscribed(true);
      setEmail("");
      setTimeout(() => setIsSubscribed(false), 3000);
    }
  };

  return (
    <footer className="w-full border-t border-gray-200 bg-linear-to-b from-gray-50 to-white mt-auto">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        {/* Main Footer Content */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          
          {/* Column 1 - Brand */}
          <div className="space-y-4">
            <Link href="/" className="inline-block group">
              <h3 className="text-3xl font-black text-[#D9006C] group-hover:text-[#B80059] transition-colors">
                STYLA
              </h3>
            </Link>
            <p className="text-sm text-gray-600 leading-relaxed max-w-xs">
              Modern fashion essentials with minimal boldness design. 
              Khám phá phong cách của riêng bạn.
            </p>
            
            {/* Contact Info */}
            <div className="space-y-2 pt-2">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
                <a href="tel:0355329485" className="hover:text-[#D9006C] transition-colors">
                  0355 329 485
                </a>
              </div>
              <div className="flex items-start gap-2 text-sm text-gray-600">
                <svg className="w-4 h-4 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                <span>Nam Từ Liêm, Hà Nội</span>
              </div>
            </div>
            
            {/* Social Links */}
            <div className="flex gap-3 pt-2">
              <a
                href="https://www.facebook.com/nhunguyttttne"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 rounded-full bg-gray-100 hover:bg-[#D9006C] text-gray-600 hover:text-white flex items-center justify-center transition-all duration-300"
                aria-label="Facebook"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
              <a
                href="https://www.instagram.com/_nguytdeyy_/"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 rounded-full bg-gray-100 hover:bg-[#D9006C] text-gray-600 hover:text-white flex items-center justify-center transition-all duration-300"
                aria-label="Instagram"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                </svg>
              </a>
              <a
                href="https://twitter.com"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 rounded-full bg-gray-100 hover:bg-[#D9006C] text-gray-600 hover:text-white flex items-center justify-center transition-all duration-300"
                aria-label="Twitter"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                </svg>
              </a>
            </div>
          </div>

          {/* Column 2 - Mua sắm */}
          <div>
            <h4 className="font-bold text-gray-900 mb-5 text-sm uppercase tracking-wider">
              Mua sắm
            </h4>
            <ul className="space-y-3">
              <li>
                <Link
                  href="/new-arrivals"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Hàng mới về
                </Link>
              </li>
              <li>
                <Link
                  href="/shop"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Tất cả sản phẩm
                </Link>
              </li>
              <li>
                <Link
                  href="/categories/women"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Thời trang nữ
                </Link>
              </li>
              <li>
                <Link
                  href="/categories/men"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Thời trang nam
                </Link>
              </li>
              <li>
                <Link
                  href="/sale"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Sale off
                </Link>
              </li>
            </ul>
            
            {/* Phụ kiện submenu */}
            <h5 className="font-semibold text-gray-900 mt-6 mb-3 text-sm">
              Phụ kiện
            </h5>
            <ul className="space-y-2">
              <li>
                <Link
                  href="/categories/accessories/shoes"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Giày dép
                </Link>
              </li>
              <li>
                <Link
                  href="/categories/accessories/hats"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Mũ
                </Link>
              </li>
              <li>
                <Link
                  href="/categories/accessories/plushies"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Gấu bông
                </Link>
              </li>
              <li>
                <Link
                  href="/categories/accessories/bags"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Túi sách
                </Link>
              </li>
            </ul>
          </div>

          {/* Column 3 - Hỗ trợ */}
          <div>
            <h4 className="font-bold text-gray-900 mb-5 text-sm uppercase tracking-wider">
              Hỗ trợ
            </h4>
            <ul className="space-y-3">
              <li>
                <Link
                  href="/contact"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Liên hệ
                </Link>
              </li>
              <li>
                <Link
                  href="/shipping"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Chính sách vận chuyển
                </Link>
              </li>
              <li>
                <Link
                  href="/returns"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Đổi trả & Hoàn tiền
                </Link>
              </li>
              <li>
                <Link
                  href="/faq"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Câu hỏi thường gặp
                </Link>
              </li>
              <li>
                <Link
                  href="/size-guide"
                  className="text-sm text-gray-600 hover:text-[#D9006C] transition-colors inline-block"
                >
                  Hướng dẫn chọn size
                </Link>
              </li>
            </ul>
          </div>

          {/* Column 4 - Newsletter */}
          <div>
            <h4 className="font-bold text-gray-900 mb-5 text-sm uppercase tracking-wider">
              Đăng ký nhận tin
            </h4>
            <p className="text-sm text-gray-600 mb-4 leading-relaxed">
              Nhận thông tin về sản phẩm mới và ưu đãi độc quyền
            </p>
            <form onSubmit={handleSubscribe} className="space-y-3">
              <div className="relative">
                <input
                  type="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  placeholder="Email của bạn"
                  required
                  className="w-full px-4 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#D9006C]/30 focus:border-[#D9006C] transition-all"
                />
              </div>
              <button
                type="submit"
                className="w-full px-4 py-2.5 bg-[#D9006C] text-white text-sm font-semibold rounded-lg hover:bg-[#B80059] transition-colors duration-300 shadow-sm hover:shadow-md flex items-center justify-center gap-2"
              >
                {isSubscribed && <Check className="w-4 h-4" />}
                {isSubscribed ? "Đã đăng ký!" : "Đăng ký ngay"}
              </button>
            </form>
            {isSubscribed && (
              <p className="text-xs text-green-600 mt-2 animate-fade-in">
                Cảm ơn bạn đã đăng ký!
              </p>
            )}
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="mt-12 pt-8 border-t border-gray-200">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-sm text-gray-500">
              © {new Date().getFullYear()} STYLA. All rights reserved.
            </p>
            
            {/* Payment Methods */}
            <div className="flex items-center gap-3">
              <span className="text-xs text-gray-500">Thanh toán:</span>
              <div className="flex gap-2">
                {['Visa', 'Mastercard', 'Momo'].map((method) => (
                  <div
                    key={method}
                    className="h-7 px-3 bg-white border border-gray-200 rounded flex items-center justify-center text-xs font-semibold text-gray-700"
                  >
                    {method}
                  </div>
                ))}
              </div>
            </div>

            {/* Legal Links */}
            <div className="flex gap-6 text-sm text-gray-500">
              <Link
                href="/privacy"
                className="hover:text-[#D9006C] transition-colors"
              >
                Bảo mật
              </Link>
              <Link
                href="/terms"
                className="hover:text-[#D9006C] transition-colors"
              >
                Điều khoản
              </Link>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}